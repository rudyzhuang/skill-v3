...
**心跳字段**（**`last_heartbeat_at`** / **`elapsed_ms`**）：由各 stage 脚本在调用 agent / tool **之前**启动，每 30 s 写入一次（参见 **`code3.md` §15.1**）；**ai-dash3** 读取 `last_heartbeat_at` 展示活跃性、`started_at` 计算精确运行时长。
