...
- **给人读的摘要（归档）**：**ai-prd3** 在 **`validate-prd-review` 终检通过**后会**尝试**写入 **`.pipeline/reports/prd-implementation-summary.md`**（用自然语言汇总 **`phase_plan`** 分期数、第一期目标与功能名，与各端 **`feature_list.md`** 对齐；写入失败仅 **stderr 警告**、**不**改变终检成功）；**不**作为门闸真源，细节以 **`prd3.md` §8.8** 为准。亦可随时用 **`run.cjs report`** 按当前磁盘 **`stages.json`** 重生成（须 **`prd_review` 已完成且 `outputs.decision=passed`**）。
