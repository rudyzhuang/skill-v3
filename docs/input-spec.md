...
| **worktree** | 由 codegen 阶段为每个 feature 创建的隔离工作区（默认 **`<project_root>/.pipeline/worktrees/`** 下的 **git worktree**，见 **`docs/spec/code3.md` §7.8**；可被等价机制替代），路径写入 `stages.codegen.outputs.worktrees[].worktree_path`（**绝对路径**）。 |
