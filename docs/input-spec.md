...
- **真源与禁止项**：**`docs/prd-spec.md`** 总源头——**评审意见、讨论纪要、批注**不得默认写回该文件正文。若确需改总规，须先在 **prd-review** 结论中记录 **`suggested_prd_spec_changes`**，并在用户明确同意后回到 **prd** 流程更新 **`docs/prd-spec.md`**，再重新派生各端文件。允许在 **prd-review** 产出中引用「建议修改 prd-spec §…」的**指针式说明**。各端 **`prd.md`** 在评审阶段默认**不当作批注白板**；对端的调整应通过 **「评审结论 → 回到 prd 流程修订 → 再派生」** 对齐，避免评审与派生稿双源漂移。
