...
| --- | --- |
| **config.dev.json** | **开发环境**侧非敏感、可结构化管理的项目信息：例如 Git 相关配置（远程、默认分支、与工作流有关的选项）、**编译与构建**选项、**dev 部署**所需的云服务**资源级**配置（区域、服务标识、功能开关、环境名等），以及你希望与 **ai-publish-dev3** 门闸对齐的其它管理字段。上一版分散在各端 **`deployment_plan.json`** 里的「部署草案 / 资源级约定」类信息，**本版统一收敛到此文件（及 release 对档）**，**不再**为每端单独维护 `deployment_plan.json`。 |
...
| 模板 | 用途 |
| --- | --- |
| **`prd-spec/prd-spec.cn.md.template`** | **PRD 总源头默认模板**；创建 **`docs/inputs/prd-spec.md`** 时默认使用中文版本。 |
...
- **与给人读的 Markdown 分工**：`docs/inputs/prd-spec.md`、各端 `prd.md`、**`feature_list.md`** 等仍以自然语言或约定正文为主；**`stages.json` 承担「门闸与编排真源（项目侧）」**，内容需与各阶段实际产物一致，避免口头状态与文件状态脱节。
...
- **cjs 脚本（`*.cjs`，CommonJS）承担"确定性的事"**：
  ...
  - **诊断与日志归集**（写 `.agent-sessions/<session_id>.log`）
...
| **ai-auto3** | ai-dash2 + autorun + autorun-pro | 默认终点改为 dev deploy + smoke + report；release 不默认跟随 |
...
本版把「从某一阶段起自动往下跑」的能力收敛到一个编排 skill 名下，命名为 **ai-auto3**（与上一版的看板编排、autorun 等**概念连续**，名称换代为 3）。
...
- **终点**：**report** 完成。  
  换言之：自动编排覆盖 `design` → … → `build` → 再由 **ai-publish-dev3** 完成 `deploy`（dev）与 `smoke`，最后由 `autorun.cjs` 调用 `gen-report.cjs` 生成 **report**。

**刻意不纳入本版自动序列的内容**：

- **prd / prd-review**：仍默认由人在 **ai-prd3** 内完成；自动编排**不从 prd 或 prd-review 起步**（避免在需求未成形或评审未完成时长跑流水线）。
...
**与上一版习惯的对应关系（便于理解）**：上一版里「从评审之后一路自动跑」多由业务仓 **autorun** 或并行增强版承担；本版把这些编排语义收到 **ai-auto3** 名下，并把编排实现下沉到 **`autorun.cjs`** 这一确定性脚本（业务仓内不再放 autorun 副本），**自动终点**明确为 **dev 发布 + 冒烟 + report**，**release 不默认跟随**。
...
**与「测试失败回退」的配合（承接上一版）**：上一版测试阶段会在失败耗尽修复次数后，在状态上给出**回到 codegen 或回到 contract**等建议；编排层（本版的 **ai-auto3** 或看板类工具）负责**读该建议并路由**，而不是由测试 skill 直接越权改写无关阶段的状态。本版仍维持这一职责划分。

**退出码与 `stages.json` 的桥接**：

- 原始**进程退出码**由编排器在**会话日志**（`.agent-sessions/<session_id>.log`）中持久化，**不**直接落在 `stages.json` 字段里。
...
- **单次 Agent 会话日志**：`.agent-sessions/<session_id>.log`
- **长时间编排、tee、后台 shell 追加**：`.agent-sessions/logs/<有意义文件名>.log`（目录可随运行创建）
...
**给人看的总结**：排查问题时，优先在 **`.agent-sessions/`** 下按时间找日志；自动编排（**ai-auto3**）的摘要里应**带上一眼能定位的路径**，而不是只写「去看临时目录」。
...
- 锁文件统一放在 **`.agent-sessions/locks/<scope>.pid`**；`<scope>` 取值固定为 `pipeline / deploy-dev / deploy-release / merge-push / build / smoke`。
...
- **进度心跳**：长时阶段（`codegen` / `test` / `build` / `deploy`）的 cjs 调用封装应**每 30 秒**向 `.agent-sessions/<session_id>.log` 追加一行心跳（`alive: stage=<x>, elapsed_s=<n>, last_step=<...>`），避免人误以为卡死。
...
| --- | --- | --- |
| **prd** | 业务目标、范围、非功能约束；**`docs/inputs/prd-spec.md`** 作为总源头；可选模板与组织规范；**ai-prd3** 提供的 **config.*.json** 模板（若尚未生成则初始化） | 各端 **`docs/<端>/prd.md`** 与 **`feature_list.md`**（由总规派生）；**`docs/config.dev.json`** / **`docs/config.release.json`** 中非敏感环境与部署资源级字段的初稿或补全建议（**替代**各端 `deployment_plan.json`）；写入 **`.pipeline/stages.json`** 的 prd 状态，并可同步到 skill 目录 DB 缓存 |
...
| **codegen** | 契约已通过机器校验 | **实现代码**（在隔离工作区或等价机制中）、代码生成结果；**严禁**回头改契约文件（上一版称 diff-guard）；写入 **`.pipeline/stages.json`** 的 codegen 状态，并可同步到 skill 目录 DB 缓存 |
...
- 标记 **destructive** 的阶段，无论手工还是 ai-auto3 自动，都**必须**有 explicit confirm（手工 = 用户确认；ai-auto3 = 配置中显式 opt-in）；缺失即按退出码 1 阻断。
...
**打算管的事**：把用户的原始想法收敛为**唯一总源头** **`docs/inputs/prd-spec.md`**；在校验通过后，按 **prd-spec** 中划定的「端」派生各端的 **`prd.md`** 与 **`feature_list.md`**；同时初始化或补全 **`docs/config.dev.json`** / **`docs/config.release.json`**（非敏感、无密钥），并生成 **`docs/config.env`** 占位文件（只含密钥名与说明，不含真实密钥），再把本阶段门闸状态写入 **`.pipeline/stages.json`**。**不再**为各端维护 **`deployment_plan.json`**——部署资源级草案由两份 **config.*.json** 承载（与上一版各端 deployment 草案**语义替代**）。

**约束打算写清楚的方向**：

- **总源头**：**`docs/inputs/prd-spec.md`** 为需求总源头；其正文仅通过 **prd** 流程（或用户显式手工编辑）更新；**prd-review** 不得把评审批注偷偷写回该文件正文（见阶段 2）。
- **内置模板与落盘位置**：**ai-prd3** skill 内置多语言 **prd-spec** 模板，默认选择**中文模板**（**`prd-spec/prd-spec.cn.md.template`**）；首次启动时复制到 **`docs/inputs/prd-spec.md`**；已存在则**跳过复制**、不覆盖。
...
- **分端派生**：总规完整且校验通过后，按 **`docs/inputs/prd-spec.md`** 中显式声明的 **`client_targets`** 在 **`docs/<端>/`** 下生成或更新 **`prd.md`**、**`feature_list.md`**。其中 **`feature_list.md` 的结构以 `docs/templates/feature_list.md.template` 为 v0 契约**，必须保留 Metadata、Status Values、Features、Feature Details 等可解析章节；所有条目须能追溯到 **prd-spec** 中的功能或端需求。
...
- **真源与禁止项**：**`docs/inputs/prd-spec.md`** 总源头——**评审意见、讨论纪要、批注**不得默认写回该文件正文。若确需改总规，须先在 **prd-review** 结论中记录 **`suggested_prd_spec_changes`**，并在用户明确同意后回到 **prd** 流程更新 **`docs/inputs/prd-spec.md`**，再重新派生各端文件。允许在 **prd-review** 产出中引用「建议修改 prd-spec §…」的**指针式说明**。各端 **`prd.md`** 在评审阶段默认**不当作批注白板**；对端的调整应通过 **「评审结论 → 回到 prd 流程修订 → 再派生」** 对齐，避免评审与派生稿双源漂移。
- **读取输入**：**`docs/inputs/prd-spec.md`**、各端 **`prd.md`** / **`feature_list.md`**、**`docs/config.dev.json`** / **`docs/config.release.json`**（若已生成）、以及 **`.pipeline/stages.json`** 中 **prd** 的完成记录与校验摘要。
...
- **手工重复跑**：若将覆盖已有评审结论或 **stages.json** 中 `stages.prd_review` 块，须按 §8.1「执行方式上的总体约定」提示用户并获**明确同意**后再执行。
...
- **契约产物（5 类，固定映射）**：本阶段必须产出且仅产出下列 5 类契约，写入 `stages.contract.outputs.artifacts[*]` 对应字段：
  ...
  | 测试规格 | `test_spec` | `<feature>.test-spec.md` / `.yaml` | 用例与验收边界，供 codegen 生成测试代码 |
...
- **隔离实现**：建议为每个功能使用独立 worktree/分支或等价隔离空间，避免未完成代码污染主线。
- **只改允许范围**：实现应限制在设计/契约声明的新建和修改文件范围内；需要新增未声明公共 API 或关键文件时，应回到 design/contract。
- **契约保护**：严禁修改契约产物；若实现过程或修复过程改动契约，应视为契约破坏，按退出码 5 或等价状态阻断。
- **测试代码同步**：契约中要求的测试文件或测试骨架，应在本阶段生成或补齐；无测试规格时必须在状态中说明为何跳过。
- **状态写入**：在 **`.pipeline/stages.json`** 中记录 worktree/分支、变更文件、实现状态、测试代码生成状态、提交信息或待提交说明；完成条件为 codegen 状态完成且实现与测试生成均成功或有明确可接受的跳过原因。
...
- **build 产物 → deploy 服务的映射规则**：deploy 按 `(client_target, sub_platform)` 元组在 `stages.build.outputs.artifacts[]` 中查找对应 `artifact_path`；若 `docs/config.dev.json.deploy.services[].artifact_ref` 显式指向某 artifact（如 `"<client_target>:<sub_platform>"`），以此为准。一个 service 必须能且只能匹配到一个 artifact，否则以退出码 1 失败。
...
- **状态写入**：在 **`.pipeline/stages.json`** 写入 smoke 结果、检查项、失败列表、耗时和跳过原因；通过后可进入 report，release 流程也可将其作为正式发布前置。
...
下列对照表覆盖上一版（v2）已知的项目侧产物文件与本机数据库表，**仅供一次性脚本化迁移参考**；本版 ai-*3 **不**做自动迁移，老项目需要人工或迁移脚本（建议放在业务仓 `scripts/migrate-v2-to-v3.cjs`）执行后**删除**旧文件。列名 / 路径以 v2 实际安装目录 `~/.cursor/skills/<skill>/templates/` 内的模板与 SQL 迁移文件为准。
...
| 所有 `*_state.feature_id` | `stages.<stage>.outputs.<sub>[].feature_id` 或 `stages.<stage>.review.phase_plan[*].feature_ids[]` | v3 把 feature 维度从行结构改为子数组，迁移脚本需按 stage 不同决定写入位置 |
...
`scripts/migrate-v2-to-v3.cjs`（业务仓内一次性脚本）建议按下列顺序执行：
...
| **worktree** | 由 codegen 阶段为每个 feature 创建的隔离工作区（默认 `git worktree`，可被等价机制替代），路径写入 `stages.codegen.outputs.worktrees[].worktree_path`。 |
...
| **session_id** | 单次 Agent / 编排会话的唯一 ID，决定日志路径 `.agent-sessions/<session_id>.log` 与锁内容。 |
