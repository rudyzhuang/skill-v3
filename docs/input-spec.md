...
| **`ai-prd3/docs/spec/prd3.md` §1.4** | req 品牌/图标须落 prd-spec feature |

**与 §4.3 默认序列的关系**：soak 在 **ai-prd3 完成后**调用 autorun；strict 下 **不得**因 `summary_hash` 跳过发布与 UI 验收阶段（**覆盖** §4.4 的常规跳过规则，见 **`auto3.md` §6.4**）。
