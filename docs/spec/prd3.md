...
2. 否则：  
   `project_id = "p-" + random_uuid 去掉连字符后取前 12 位十六进制字符`（与 v1 模板示例语义一致即可）。
...
`prd-spec.cn.md.template` 要求：已声明的端必须在 **「## 7. 各端专属需求」** 下有对应 **`### <slug>`** 小节且内容已补齐。**`prd-validate-spec.cjs`** 须对每个 `declared` 端验证该小节存在；启发式阈值写在脚本常量，并由 **§12** 用例覆盖。
...
2. **LLM**（`prompts/prd-review.md`）：产出 **结构化 JSON**（由脚本 schema 校验），**禁止** LLM 直接整文件改写 `stages.json`。  
3. **`prd-review-write-stage.cjs`**：合并 JSON 到 `stages.prd_review`；维护 `review.*`、`outputs.decision`、`conditions`、`blocking_issues`、`validation.*`；写 **§9.2** 哈希。  
4. **`prd-review-validate.cjs`（终检）**：满足 **§8.4** 全部条件才可称「门闸通过」。
...
2. `blocking_issues` 为空数组；若曾 `conditional_passed`，须已落实所有 `conditions` 且 **`outputs.decision` 已改写为 `passed`**（仍须满足本条其余项）；  
3. **本期** `review.phase_plan[*].feature_ids` 合并去重后非空，且每个 `feature_id` 至少在**某一个**已声明端（`declared`）的 **`feature_list.md`** → Features 表中存在；  
4. **`validation.config_secret_scan_passed=true`**（对当前 `config.*.json` 重跑附录 B 规则）；  
...
在 **`prd-review-write-stage.cjs`** 在评审结论写入后、标记 `completed` 前计算。  
...
写入 **`stages.prd_review.inputs.summary_hash`**（占位键同 **§9.1**，须为 **64 位小写十六进制** 非空值）。
...
| 声明端但「## 7. 各端专属需求」缺对应 `### <slug>` 小节 | `validate-prd` 退出 1 |
...
| `conditional_passed` 且 `conditions` 非空 | 不满足 §8.4.1「passed」语义 |
...
2. 发布 **ai-prd3** 时将 **`templates/`** 目录与之一致版本打包（拷贝或构建脚本）。  
...
| ID | 内容 | 关闭条件 |
| --- | --- | --- |
| T1 | LLM 产出 JSON 的 **JSON Schema** 文件是否随 skill 分发 | 已定稿路径并入 §4 目录树 |
| T2 | **registry.sqlite** 是否在 prd 完成时写入 | 若写入：失败 **warn 不阻断**；若不写入则删除本行 |
...
*文档版本：与 **docs/input-spec.md**、**docs/templates/stages.json.template** v1、**docs/templates/prd-spec/prd-spec.cn.md.template**、**docs/templates/prd-spec/prd-spec.en.md.template** 当前检入对齐；变更时请走 §0 维护流程。*
