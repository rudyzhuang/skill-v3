...
[← 规范索引](../std3.md) · [门闸链](../std3.md#2-门闸链汇总) · [编排映射](../std3.md#3-run-pipelinecjs-编排映射) · [流水线收尾](../std3.md#33-流水线收尾report-之后) · [卡点速查](../std3.md#4-agent-卡点速查)
...
1. 调用 **Agent** + **`ai-std3/prompts/report-author.md`**。
...
3. Agent **产出**（写入 `.pipeline/reports/.report-agent-<datetime>.md`）：
   ...
   - **「建议的下一步」**：编号列表，每条含**可复制**的 `run-pipeline.cjs` CLI（与 [§4 卡点速查](../std3.md#4-agent-卡点速查) 一致）
4. 环境变量：`AI_STD3_REPORT_AGENT_OUTPUT`、 `AI_STD3_PROJECT`、`AI_STD3_SESSION_ID`。
...
- `outputs`：
  ...
  - `teardown`：`{ invoked: true, killed_pids[], errors[] }`（由编排器写入，见 [流水线收尾](../std3.md#33-流水线收尾report-之后)）
- **stdout**（CI / 看板）：
...
`report.cjs` **正常退出 0** 后，编排器**必须**调用 **`pipeline-teardown.cjs`**（除非 `--no-teardown`）。详见 [std3 §3.3](../std3.md#33-流水线收尾report-之后)。
...
> 业务失败时 report 仍返回 **0**，以便 CI 归档报告；编排器根据 `stages.report.outputs.overall` 决定进程退出码（见 [std3 §3.3](../std3.md#33-流水线收尾report-之后)）。
