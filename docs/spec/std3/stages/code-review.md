...
| 环境变量 `AI_STD3_AGENT_BIN` | 外部 Agent 可执行路径 |

**CLI 过滤**：`--feature=<feature_id>` 仅重评单个 feature（用于失败重跑）；仍遵守上游门闸。

> 兼容旧约定：环境变量 `AI_STD3_CODE_REVIEW_JSON` 或 CLI `--code-review-json=<path>` **已弃用**（v3 全面走 per-feature Agent 派发，不再接受 stage 级单文件覆盖）；若检测到，打 `WARN` 并忽略。
...
2. **Agent-CodeReview（按 feature 并发）**：
   ...
   - 每个 Agent **仅评审一个 feature**，按 **`ai-std3/prompts/code-review-agent.md`** 执行，产出 **`<业务项目根绝对路径>/.pipeline/code-review-<feature_id>.json`**（**不得**修改 worktree、`design.json`、`stages.json`）。
   - 单 feature 评审落盘后：
