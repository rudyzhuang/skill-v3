...
编排入口与其它 stage 相同，位于 **`ai-std3/scripts/stages/setup.cjs`**；子脚本位于 **`ai-std3/scripts/libs/`** 目录（见 [std3 §3](../std3.md#3-run-pipelinecjs-编排映射)）。
...
```bash
node ai-std3/scripts/stages/setup.cjs --project=<业务项目根绝对路径>
```

子脚本亦可单独调试：`node ai-std3/scripts/libs/verify-inputs.cjs --project=...`。
