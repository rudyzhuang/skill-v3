...
| 脚本 | 职责 |
| --- | --- |
| `build.cjs` | 编排入口：门闸 → bootstrap → 并行构建 → validate；写 `stages.build` |
| `lib/build-probe.cjs` | 只读探测：按端目录识别框架、推导默认 `command` / `cwd` / `artifact_globs`（可被 config 覆盖） |
| `lib/build-runner.cjs` | 单条 `(client_target, sub_platform)` 构建：子进程、超时、stdout/stderr 落盘、产物存在性校验 |
...
1. 获取 PID 锁（`build`），锁路径：`<项目根>/.pipeline/locks/build.pid`。
2. 校验 `merge_push` 门闸与 `final_commit`；`git checkout` / `git pull` **不在**本 stage 执行（由 merge_push 保证远端一致；本 stage 只 **校验** HEAD）。
3. 解析 `target_set[]`，展开为 `build_units[]`：每个 `client_target` × `sub_platforms[]`（未声明子平台时视为 `[{ "id": "default" }]`，与 v3 一致）。
4. 计算 `inputs.summary_hash`：SHA-256，规范化 JSON 包含 `final_commit`、`build` 相关 config 子树（`client_targets`、`commands`、`artifacts_dir`、`install_before_build`）、`build_units` 列表。
5. 若 `inputs.summary_hash` 与上次相同且全部 unit 上次 `status ∈ {completed, not_applicable, skipped}` 且 `artifact_path` 存在（可选 `artifact_check.passed`）→ `stage_skipped`，直接进入步骤 4 validate（仅刷新报告时间戳）。
6. 初始化 `stages.build`：`status=running`、`outputs.build_units_total`、`outputs.artifacts=[]`、`validation.passed=false`。
7. 打 `stage_start`；对每个 unit 预写 `outputs.artifacts[]` 行，`status=pending`。
...
3. 对单个 unit：
   ...
   - `cwd = probe_root`；`env` 注入 `AI_STD3_PROJECT_ROOT`、`AI_STD3_CLIENT_TARGET`、`AI_STD3_SUB_PLATFORM`；
...
| 码 | 含义 |
| ---: | --- |
| 0 | 全部须构建 unit 通过，或 `stage_skipped` |
| 1 | 门闸/HEAD 不一致/配置无法解析/无 target |
| 3 | 至少一个 unit **超时**（且无其它失败时可单独 3；与 failed 并存时优先 **4**） |
...
> 除 [通用事件](../std3.md#标准事件类型所有-stage-通用) 外，本 stage 扩展如下（`meta` 无额外说明时写 `{}`）。
...
| `.pipeline/stages.json` | `stages.build`（见下） |
