...
> 实现要求：固定大小 Worker 池，**禁止**按 feature 无限制起线程。评审 Agent **仅依赖 design.json 已就绪**（feature 级），与 design Agent **可同时进行**（合计占用须遵守 `pipeline.autorun.feature_max_parallel` 全局限额，见 [复合编排](../std3.md#31-design--design-review-复合编排)）。组内若槽位不足，**优先评审 `topo_order` 靠前的 feature（依赖端）**。
...
- 按 `effective_parallel` 与全局限额并发启动 Agent；每个 Agent **仅评审一个 feature**，按 **`ai-std3/prompts/design-review.md`** 执行，产出 **`.pipeline/design-review-<feature_id>.json`**（**不得**直接修改 `design.json`）。
...
| 粒度 | 条件 | 效果 |
| --- | --- | --- |
| **group → 下游** | 组内全部 feature `decision=passed` 且无 blocking gap | `released_groups[]` 追加该组；组内所有 `features.<id>.can_enter_codegen=true`；`run-pipeline` 按 [§3.1](../std3.md#31-design--design-review-复合编排) 为该组 feature 同时启动 `codegen` 与 `create-ui-scenarios` **两条并行 track**（见 [§3.2 并行编排](../std3.md#32-codegen--create-ui-scenarios-并行编排)），组内多线程，两 track 之间无相互门闸 |
