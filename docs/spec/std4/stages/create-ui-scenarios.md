...
[← 规范索引](../std4.md) · [门闸链](../std4.md#2-门闸链汇总) · [design 复合编排](../std4.md#31-design--design-review-复合编排) · [codegen 并行编排](../std4.md#32-codegen--create-ui-scenarios-并行编排) · [卡点速查](../std4.md#4-agent-卡点速查)

> 按 **feature** 派发 Agent，从 `design.json` 的 `acceptance[]` / `api_outline[]` / `client_target` 派生**可执行的 UI 测试用例 YAML**，供 [ui_e2e](ui_e2e.md) 消费。
>
> **与 codegen 并行**：本 stage **不**是 codegen 的硬前置；design-review 释放某 group 后，组内 feature **同时**进入 codegen 与 create-ui-scenarios **两条独立 track**；只要在 `ui_e2e` 启动前就绪即可（见 [§3.2](../std4.md#32-codegen--create-ui-scenarios-并行编排)）。

## 脚本

脚本根目录前缀 **`ai-std4/scripts/`**：`stages/create-ui-scenarios.cjs`（编排入口）、`libs/create-ui-scenarios-bootstrap.cjs`、`libs/create-ui-scenarios-validate.cjs`；步骤 2 为按 **feature** 并发的 Agent 池；与 `stages/codegen.cjs` 复合编排时支持 **`--tick`**。
...
2. **Agent-CreateUIScenarios（按 feature 并发，组感知）**：
   ...
   **单 feature 产出 YAML**（须满足 `docs/spec/std4/schemas/ui-scenarios.yaml.schema.json`）：
...
| **与 codegen 关系** | **无相互门闸**（见 [§3.2](../std4.md#32-codegen--create-ui-scenarios-并行编排)） | 同 group 内 feature 同时进入 codegen 与 create-ui-scenarios；codegen 完成不等待场景，反之亦然 |
