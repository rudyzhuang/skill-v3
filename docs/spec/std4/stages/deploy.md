...
```bash
node ai-std4/scripts/stages/deploy.cjs --project=<业务项目根绝对路径> [--explicit-confirm]
```
...
2. **SDK 分诊 Agent**（`CURSOR_API_KEY` + `invoke-sdk-agent.cjs`），提示词 **`ai-std4/prompts/deploy-triage.md`**，只读输入：
   - `.pipeline/deploy-last-error.json`
   - `logs/stages/deploy/<datetime>*.log`（失败 service 相关）
   - `docs/config.{dev|release}.json` 的 `deploy` 子树（无密钥）
   - **仅可读** `ai-std4/scripts/libs/providers/cloudflare.cjs` 与 `stages/deploy.cjs`（用于判断脚本缺陷）
...
| --- | --- | --- |
| `fix_script` | 分诊 Agent **同轮**修改 `ai-std4/scripts/**` deploy 相关脚本；`agent_fix_attempts += 1` 后 **本 stage 内**对失败 service 调用 `retryOneService`（与 `retry_deploy` 相同路径）；用尽 → **4** | 用尽仍失败 → exit **4**（编排级 recovery 可再兜底） |
...
**退出码 4 vs 8 vs 9**（deploy 专用，见 [std4 退出码](../std4.md#退出码)）：
...
| `status=failed` + 退出码 **4/8** | 按 [卡点速查](../std4.md#4-agent-卡点速查) 修复后重跑 |
