...
[← 规范索引](../std4.md) · [门闸链](../std4.md#2-门闸链汇总) · [codegen 并行编排](../std4.md#32-codegen--create-ui-scenarios-并行编排) · [卡点速查](../std4.md#4-agent-卡点速查)
...
>
> **与 create-ui-scenarios 并行**：本 stage 与 [create-ui-scenarios](create-ui-scenarios.md) **无相互门闸**，design-review 释放某 group 后两条 track 同时启动；调度见 [§3.2](../std4.md#32-codegen--create-ui-scenarios-并行编排)。

## 脚本

脚本根目录前缀 **`ai-std4/scripts/`**：`stages/codegen.cjs`（编排入口，支持 **`--tick`**）、`libs/codegen-bootstrap.cjs`、`libs/codegen-worker.cjs`（长驻 worker）、`libs/codegen-validate.cjs`、`libs/http-smoke.cjs`（内联 smoke）。

```bash
node ai-std4/scripts/stages/codegen.cjs --project=<业务项目根绝对路径> [--tick] [--feature=<feature_id>]
```
...
**`--tick`**：单轮调度后返回，由 `run-pipeline.cjs` 与 `create-ui-scenarios --tick` 交替调用（见 [§3.2](../std4.md#32-codegen--create-ui-scenarios-并行编排)）。
...
2. **`codegen-worker.cjs`（单 feature 长驻 worker，feature 并发 + 看门狗 + 心跳 + resume）**：
   ...
   worker init
     ...
     ├─ 启动 Agent 子进程：
     │     - attempt_index == 1 → prompt = ai-std4/prompts/codegen-impl.md
     │     - attempt_index >= 2 → prompt = ai-std4/prompts/codegen-impl-resume.md
     │     - cwd = worktree_path
   ...
   2. 拉起新 Agent 子进程，`prompt = ai-std4/prompts/codegen-impl-resume.md`，并通过 prompt 模板强制要求 Agent：
...
| **与 create-ui-scenarios 关系** | **无相互门闸**（见 [§3.2](../std4.md#32-codegen--create-ui-scenarios-并行编排)） | 同 group 内 feature 同时进入两条 track；codegen 完成不等待场景，反之亦然 |
