...
| `output-stages/stages.json` | `stages.prd.outputs.features[]`（跨端 feature 全集，评审覆盖门闸的真源） |
...
| `output-stages/stages.json` | `stages.prd_review` 完成态：`outputs.decision`、`review.phase_plan[]`、`review.deferred_features[]` |
