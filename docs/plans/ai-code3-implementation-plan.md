...
| **P4** | 5 | **`test.cjs`**：Agent 式 fix-loop 定稿 + **`rollback_to`** 细化 | **完成** | 按 **feature worktree**；**`AI_CODE3_PHASE=test_fix`**；**`AI_CODE3_SKIP_TEST_FIX_AGENT`** |
| **P5** | 6 | **`code-review.cjs`**：契约驱动结构化输出 + JSON Schema（§18 T2） | **完成** | **Ajv** + **`templates/schemas/code-review-output.v3.schema.json`**；**`AI_CODE3_CODE_REVIEW_OUTPUT`** |
...
| **P8** | 9 | **§15 心跳** | **完成** | **merge_push** 已纳入；**`--session-id=`** |
| **P9** | 10 | **`prompts/*.md`**、`clean` 子命令 | **完成** | **`clean.cjs`**、**`AI_CODE3_CLEAN_CONFIRM`** |
| **P10** | 11 | **§16 自动化验收矩阵** | **完成** | **`smoke.cjs`** 内含 **`npm ci`** + **clean** + **preflight 上游** 自测 |
...
| 3 | `node ai-code3/scripts/smoke.cjs`（内含 **`npm ci`**、`self-test-clean`、`self-test-preflight-upstream`） | PASS | PASS |
...
| 2 | **`docs/spec/code3.md` §4.1** 目录树与 **`self-test-*.cjs`** 清单齐全；脚本职责表与 **§0.1** 一致 | OK |
...
| test + rollback_to | §9 | **完成** | **per-feature worktree**；**`test_fix`** + **`AI_CODE3_PHASE=test_fix`** Agent |
| code-review | §10 | **完成** | **JSON 导入** 优先；否则 **Agent + Ajv**（§18 T2） |
| merge-push 真 merge / 6 / push 7 | §11 | **完成** | `merge-git.cjs`；**`--session-id`** 心跳 |
...
| 附录 B 单测 | 附录 B | **完成** | |
| §15 心跳 | §15 | **完成** | codegen / test / build / **merge_push** + **`--session-id=`** |

## 5. 已知局限（与 SSOT 一致）

- **codegen**：**§7.8–§7.12** 中与编排、多相 Cursor 集成的**穷尽**验收仍以 SSOT 为准。  
- **test**：**`rollback_to`** 语义可继续与 **`input-spec.md` §7.1** 对齐细化。  
...
| R6 | 2026-05-16 | **P4–P10** 收口：`smoke`（**`npm ci`** + clean + preflight 上游）×1；**§18 T2**；**`code3.md` §0.1** | **PASS** |
| R7 | 2026-05-16 | **全量评审 R1**：§4.1 自动化 ×1 + 文档 §0.1/§4.1/§4.2/§4.3 核对 | **PASS** |
| R8 | 2026-05-16 | **全量评审 R2**：与 R7 **相同**三条命令再跑 ×1（连续第二轮） | **PASS** |
| R9 | 2026-05-16 | **全量评审 R1（复评）**：§4.1 自动化 ×1 + 对照 **`code3.md` §0.1** | **PASS** |
| R10 | 2026-05-16 | **全量评审 R2（复评）**：与 R9 **相同**三条命令 ×1（连续第二轮） | **PASS** |

**结论**：**§4.1** 自动化**最近连续两轮**（**R9 / R10**）**exit 0**；**§0.1 / §4.1 / §4.3 / §11.4** 与 **`ai-code3/scripts`** 一致；**codegen** 与 **§7.8–§7.12** 的**细项穷尽**仍以 SSOT + **§4.3**「部分」为准。
