...
function parseRunArgs(argv, ctx) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    else if (a === '--explicit-confirm') args.explicitConfirm = true;
    else if (a === '--confirm-deploy') args.confirmDeploy = true;
    else if (!a.startsWith('-')) args._.push(a);
  }
  if (ctx.environment === 'dev' && args.confirmDeploy) {
    console.error('ai-publish-dev3 不接受 --confirm-deploy（该开关仅用于 ai-publish-release3）。');
    process.exit(1);
  }
  return args;
}
