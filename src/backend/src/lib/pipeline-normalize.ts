...
function collectFeaturesFromStages(
  stagesMap: Record<string, unknown>,
  nameLookup: Map<string, string>,
): NormalizedFeatureRow[] {
  const rows: NormalizedFeatureRow[] = [];
  ...
  for (const [stageId, stageVal] of Object.entries(stagesMap)) {
    const stageRec = asRecord(stageVal);
    const featuresMap = asRecord(stageRec?.features);
    if (!featuresMap) {
      continue;
    }
    for (const [featureId, raw] of Object.entries(featuresMap)) {
      ...
      rows.push({
        ...
        phase: asString(rec.phase),
        status: asString(rec.status) ?? 'unknown',
        current_stage: asString(rec.current_stage) ?? stageId,
        dependencies: deps,
      });
    }
  }
...
/** Normalize pipeline snapshot or stages.json-shaped payloads. */
export function normalizePipelinePayload(raw: unknown): NormalizedPipelinePayload {
  ...
  if (features.length === 0) {
    const stagesMap = asRecord(stagesRaw);
    if (stagesMap) {
      features = collectFeaturesFromStages(stagesMap, nameLookup);
    }
  }
