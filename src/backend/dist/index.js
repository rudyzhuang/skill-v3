...
__export(schema_exports, {
  ...
  listUsers: () => listUsers,
  toUserListItem: () => toUserListItem,
  toUserSummary: () => toUserSummary,
...
async function updateUserFields(db, id, fields) {
  ...
  bindings.push(id);
  await db.prepare(`UPDATE users SET ${sets.join(", ")} WHERE id = ?`).bind(...bindings).run();
  return findUserById(db, id);
}
...
var USER_ROLES, USER_STATUSES, ADMIN_ROLES;
var init_schema = __esm({
  "src/db/schema.ts"() {
    "use strict";
    USER_ROLES = ["super_admin", "admin", "operator"];
    USER_STATUSES = ["active", "disabled"];
    ADMIN_ROLES = ["admin", "super_admin"];
    __name(toUserSummary, "toUserSummary");
    __name(toUserListItem, "toUserListItem");
    ...
    __name(findSessionById, "findSessionById");
    __name(insertSession, "insertSession");
    __name(deleteSession, "deleteSession");
    __name(findProjectById, "findProjectById");
    __name(insertProject, "insertProject");
  }
});
...
// src/services/bootstrap-admin.ts
init_schema();
...
init_schema();
var SESSION_COOKIE = "dash_session";
...
function buildSessionCookie(sessionId, expiresAt, opts) {
  const segments = [
...
async function resolveSessionUser(db, sessionId) {
  if (!sessionId) {
    return null;
  }
  ...
  const { findUserById: findUserById2 } = await Promise.resolve().then(() => (init_schema(), schema_exports));
  const user = await findUserById2(db, session.user_id);
  if (!user || user.status !== "active") {
    return null;
  }
  return user;
}
__name(resolveSessionUser, "resolveSessionUser");
...
// src/services/auth.ts
var LOGIN_ERROR_MESSAGE = "\u90AE\u7BB1\u6216\u5BC6\u7801\u4E0D\u6B63\u786E";
function cookieOptionsFromEnv(env) {
  return {
...
__name(cookieOptionsFromEnv, "cookieOptionsFromEnv");
async function loginWithCredentials(env, email, password) {
...
var ADMIN_WRITE_ROLES = /* @__PURE__ */ new Set(["admin", "super_admin"]);
async function requireAdminRole(c, next) {
...
__name(requireAdminRole, "requireAdminRole");
...
// src/services/projects-create.ts
init_schema();
function newProjectId() {
  return crypto.randomUUID();
}
__name(newProjectId, "newProjectId");
async function createProjectForAdmin(db, input) {
  const now = (/* @__PURE__ */ new Date()).toISOString();
...
__name(createProjectForAdmin, "createProjectForAdmin");

// src/types/project.ts
var ALLOWED_CLIENT_TARGETS = ["admin", "backend"];

// src/lib/client-targets.ts
var ALLOWED_SET = new Set(ALLOWED_CLIENT_TARGETS);
function parseClientTargetsJson(raw2) {
  try {
...
__name(parseClientTargetsJson, "parseClientTargetsJson");
function normalizeClientTargets(raw2) {
  const parsed = parseClientTargetsJson(raw2);
...
__name(normalizeClientTargets, "normalizeClientTargets");

// src/types/project-summary.ts
var PROJECT_STATUSES = ["active", "blocked", "completed", "unknown"];
var ALLOWED_CLIENT_TARGETS2 = ["admin", "backend"];

// src/services/projects-list.ts
function toStatus(raw2) {
  if (PROJECT_STATUSES.includes(raw2)) {
    return raw2;
  }
  return "unknown";
}
...
async function listProjectsForAdmin(db, query) {
...
__name(listProjectsForAdmin, "listProjectsForAdmin");
function parseAdminListQuery(searchParams) {
  const statusRaw = searchParams.get("status");
  let status;
  if (statusRaw !== null && statusRaw !== "") {
    if (!PROJECT_STATUSES.includes(statusRaw)) {
      return {
...
__name(parseAdminListQuery, "parseAdminListQuery");

// src/validators/project-create.ts
var ALLOWED_SET2 = new Set(ALLOWED_CLIENT_TARGETS2);
function validateCreateProjectBody(body) {
  ...
  if (!Array.isArray(record.client_targets)) {
    ...
    for (const item of record.client_targets) {
      if (typeof item !== "string" || !ALLOWED_SET2.has(item)) {
        errors.push(
          `client_targets \u4EC5\u5141\u8BB8 ${ALLOWED_CLIENT_TARGETS2.join("\u3001")}\uFF0C\u4E14\u81F3\u5C11\u4E00\u9879`
        );
        break;
      }
      if (!seen.has(item)) {
...
__name(validateCreateProjectBody, "validateCreateProjectBody");

// src/routes/projects.ts
var projectsRoutes = new Hono2();
...
function createApp() {
  ...
  app2.route("/api/projects", projectsRoutes);
  return app2;
}
