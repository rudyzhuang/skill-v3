...
| **mobile 环境不满足** | ai-e2e3 输出 `blocker=mobile_env_unsatisfied`：无真机/模拟器连接且 `flutter emulators` 为空，或 Flutter/Xcode/AVD 未安装 | 提供：`flutter devices` / `flutter emulators` 完整输出 + 用户需安装的组件（AVD 或 Xcode Simulator） |
...
- 规范详见 **`docs/spec/soak3.md` §5.0** 与 **`docs/spec/rfc-soak3-req-fidelity.md`**。
- 调用 **ai-auto3** 时须等价于 **`--force-rerun=codegen,build,deploy,smoke,ui_e2e`**（或依赖 autorun 读取 `AI_SOAK3_STRICT`）。
- **B 之后、§6 之前**执行 **`soak3.md` §10 手工门闸**（H1–H7）；脚本未实现 strict 时也不得省略。
...
export http_proxy=http://127.0.0.1:1087 https_proxy=http://127.0.0.1:1087
export AI_SOAK3_STRICT=1
# 实现落地后 autorun 将读取 AI_SOAK3_STRICT；此前须显式：
node ~/.cursor/skills/ai-auto3/scripts/autorun.cjs --project=PROJECT_ROOT \
  --force-rerun=codegen,build,deploy,smoke,ui_e2e
echo "autorun exit: $?"
```
...
### 6.1a Website / Admin 内容指纹（必做）

不仅对 status code：**响应体**须含 prd/req 声明关键词，且 **不得**含错站特征（如 `TiddlyWiki`）。见 **`soak3.md` §6.2.1**。

```bash
/usr/bin/curl -sS --max-time 15 "https://<domain>/website/" | grep -q "笔记"   # 示例
/usr/bin/curl -sS --max-time 15 "https://<domain>/website/" | grep -q "TiddlyWiki" && exit 1
```
...
- app 启动不崩溃 → 必要但不充分。
- **§6.3.1**：显示名须为 req 中的「真实笔记」/ RealNotes，**禁止**仍为 Health Mobile / health_mobile。
- **§6.3.2**：须有笔记 CRUD UI，不得仅为 Health API 探活壳。
...
1. **A**：prd + prd_review `passed`；`client_targets` 与 `inputs/req.md` 一致；**prd-spec 覆盖 req 每一条功能需求**（含品牌名、图标）。
...
*规格变更请走 `docs/spec/soak3.md` 与 `docs/spec/rfc-soak3-req-fidelity.md` 维护流程。*
