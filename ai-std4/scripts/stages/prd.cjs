 ...
 * @param {string} opts.promptFile   - 相对 ai-std4/prompts/ 的文件名
...
async function runAgentBForTarget({ clientTarget, model, timeoutMs, maxRetries }) {
  ...
  // 拷贝模板（若目标文件不存在）
  const templatePath = path.join(skillsRoot, 'ai-std4', 'templates', resolved.template);
...
async function main() {
  ...
  await gitStageSync.finalizeStageGit(projectRoot, 'prd', {
