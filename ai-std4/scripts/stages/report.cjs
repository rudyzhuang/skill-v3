...
async function invokeReportAgent(collectJsonPath, excerptPath, datetime) {
  ...
  try {
    ...
    // 设置环境变量供 Agent 读取
    process.env.AI_STD4_REPORT_AGENT_OUTPUT = agentOutPath;
    process.env.AI_STD4_PROJECT             = projectRoot;
    process.env.AI_STD4_SESSION_ID          = sessionId;
    ...
    if (overall === 'success') {
      ...
      if (failedSks.length > 0) {
        failedSks.forEach((sk, i) => {
          tips.push(`${i + 1}. 修复 ${sk} 后重跑：\`node ai-std4/scripts/run-pipeline.cjs --project=${projectRoot} --from-stage=${sk.replace(/_/g, '-')}\``);
        });
      } else if (stoppedInfo) {
        tips.push(`1. 续跑：\`node ai-std4/scripts/run-pipeline.cjs --project=${projectRoot} --from-stage=${(stoppedInfo.stopped_stage || 'setup').replace(/_/g, '-')}\``);
      } else {
