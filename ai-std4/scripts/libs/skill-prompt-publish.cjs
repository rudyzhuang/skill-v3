 ...
 * @param {string[]} [opts.files] 相对 ai-std4/ 的路径，如 prompts/codegen-impl.md
...
function publishSkillPrompts(opts) {
  ...
  const aiStd4Dir = fs.existsSync(path.join(repoRoot, 'ai-std4'))
    ? path.join(repoRoot, 'ai-std4')
    : repoRoot;

  const relFiles = (opts.files || [])
    .map((f) => String(f).trim().replace(/^ai-std4\//, ''))
    .filter((f) => f && f.startsWith('prompts/') && !isBlocked(f));

  if (relFiles.length === 0) {
    const promptsDir = path.join(aiStd4Dir, 'prompts');
    if (!fs.existsSync(promptsDir)) {
      return { ok: false, commit: null, pushed: false, files: [], error: '无 prompts/ 目录或无变更文件' };
    }
    const r = runGit(repoRoot, ['add', '--', 'ai-std4/prompts/']);
    if (r.status !== 0 && !fs.existsSync(path.join(repoRoot, 'prompts'))) {
      runGit(aiStd4Dir, ['add', '--', 'prompts/']);
    }
  } else {
    for (const rel of relFiles) {
      const abs = path.join(aiStd4Dir, rel);
      if (!fs.existsSync(abs)) continue;
      if (fs.existsSync(path.join(repoRoot, 'ai-std4'))) {
        runGit(repoRoot, ['add', '--', path.join('ai-std4', rel).replace(/\\/g, '/')]);
      } else {
        runGit(aiStd4Dir, ['add', '--', rel]);
      }
    }
  }
