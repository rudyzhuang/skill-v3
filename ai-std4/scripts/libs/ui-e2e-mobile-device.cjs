...
function log(msg) {
  console.error(`[ai-std4][mobile] ${msg}`);
}
...
/**
 * 设备 + 构建 + 安装（不跑全量 integration_test），供单场景 runner 复用。
 */
async function prepareMobileSession(projectRoot, platform, config) {
 ...
 * @param {string} testRel integration_test/xxx_test.dart
 */
async function runIntegrationTestFile(mobileDir, deviceId, testRel) {
  log(`运行 ${testRel} @ ${deviceId}`);
  const r = await runWithTimeout('flutter', ['test', testRel, '-d', deviceId], {
    cwd: mobileDir,
    timeoutMs: 600000,
  });
  const output = `${r.stdout || ''}\n${r.stderr || ''}`;
  if (r.timedOut) return { ok: false, error: 'flutter test 超时', output };
  if (r.code !== 0) {
    return { ok: false, error: (r.stderr || r.stdout || 'flutter test failed').slice(0, 800), output };
  }
