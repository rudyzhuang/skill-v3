...
/**
 * run-dash.cjs — ai-std3 流水线状态 TUI 看板
 *
 * 调用形态：
 *   node ai-std3/scripts/run-dash.cjs \
 *     --project=<业务项目根绝对路径> \
...
if (!fs.existsSync(stagesJsonPath)) {
  process.stderr.write(
    `[run-dash] 未找到已初始化的 std3 项目：${stagesJsonPath}\n` +
    `  请先运行 setup：node ai-std3/scripts/run-pipeline.cjs --project=${projectRoot}\n`
  );
  process.exit(1);
}
...
function runFallbackMode() {
  function printStatus() {
    ...
    process.stdout.write(
      `\n=== ai-std3 流水线状态 [${now.toLocaleString('zh-CN')}] 项目: ${projectName} ===\n`
    );
