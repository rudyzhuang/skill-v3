...
/**
 * run-pipeline.cjs — ai-std3 流水线主调度入口
 *
 * 调用形态：
 *   node ai-std3/scripts/run-pipeline.cjs \
 *     --project=<路径>            # 业务项目根（必须；或 AI_STD3_PROJECT 环境变量；或 cwd）
...
// 项目路径（优先级：--project > AI_STD3_PROJECT > cwd）
...
// ── 路径常量 ──────────────────────────────────────────────────────
const SCRIPTS_DIR    = __dirname;                               // ai-std3/scripts/
const STAGES_DIR     = path.join(SCRIPTS_DIR, 'stages');        // ai-std3/scripts/stages/
