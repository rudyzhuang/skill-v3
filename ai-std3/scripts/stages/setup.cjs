...
async function main() {
  ...
  if (!stagesObj) {
    // 从模板拷贝并初始化
    const templatePath = path.join(skillsRoot, 'ai-std3', 'templates', 'stages.json.template');
    let template;
    try {
      template = JSON.parse(fs.readFileSync(templatePath, 'utf8'));
    } catch (_) {
      template = { pipeline: { current_stage: null, last_completed_stage: null, updated_at: null, updated_by: 'ai-std3', project: { project_id: null, root_path: null, name: null, git: { remote: null, remote_url: null, default_branch: null, repo_initialized_at: null, remote_configured_at: null } } }, stages: {} };
    }
    ...
    stagesObj = template;
    stagesObj.pipeline.updated_by = 'ai-std3';
  ...
  if (!verifyResult.passed) {
    ...
    console.error(`\n补全后运行：node ai-std3/scripts/stages/setup.cjs --project=${projectRoot}`);
    process.exit(2);
  }
